...
`chunker.go`（格式常量）。当前写入版本 **v9**，可读取 **v2 ~ v9**。
...
`version = 9`，`verCompat = 8`。

**只有归档内确实存在符号链接时才写 v9**；普通归档仍写 v8，
这样旧版二进制照样能解开新工具打的包。含链接的归档被旧版读取时报
`版本不兼容: 9`——**明确失败，而不是解出错误数据**。

### 兼容性保证

- 读：v2 ~ v9 全部可读（旧块表走逐块 16B 哈希校验，v6+ 走流级校验）
- 写：默认为 v8（兼容优先），必要时升 v9
